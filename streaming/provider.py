...
async def search_v2(title: str, year: str, is_movie: bool):
    matches = []
    try:
        s = SessionV2(proxy=PROXY_URL or None)
...
async def search_v1(title: str, year: str, is_movie: bool):
    matches = []
    try:
        s = SessionV1(proxy=PROXY_URL or None)
...
async def search_v3(title: str, year: str, is_movie: bool):
    ...
    try:
        ...
        # Use one session for the search itself
        search_session = SessionV3(proxy=PROXY_URL or None)
        await search_session.start()
        sv = SearchV3(search_session, query=title, subject_type=st, per_page=10)
        res = await sv.get_content_model()
        count = 0
        for item in res.items:
            if not year or str(item.release_date.year) == str(year):
                # Give each match its OWN session so fetch_v3 can close it
                # without killing other matches' sessions
                item_session = SessionV3(proxy=PROXY_URL or None)
                await item_session.start()
                sessions_to_close.append(item_session)
                matches.append({"item": item, "session": item_session, "version": "v3"})
                count += 1
                if count >= 3:
                    break
    except Exception:
        pass
    # Close the search session (no longer needed) — keep item sessions open
    # for extract_streams to use
    try:
...
def extract_match_language_info(match: dict) -> dict:
    ...
    audio_lang = None
    is_dubbed = False  # True if title has [Language] suffix (indicates a dub)
    subtitle_langs = []
    seen_subs = set()
    ...
    if lang_from_title:
        ...
        is_dubbed = True  # Title suffix means this is a dubbed version
    else:
        # No title suffix → this is likely the original-audio version.
        # Use the V3 API's `language` field (list[str]) to identify the
        # original language, so we can display "🔊 English Audio" etc.
        # instead of showing nothing.
        item_langs = getattr(item, "language", None)
        if item_langs and isinstance(item_langs, list) and len(item_langs) > 0:
            first = item_langs[0]
            if isinstance(first, str) and first.strip():
                audio_lang = first.strip()
        # is_dubbed stays False — this is original audio
