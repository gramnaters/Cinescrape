...
async def handle_stream(request: Request, type: str, id: str, config_str: str):
    ...
    def sort_key(x):
        ...
        is_dubbed = x.get("is_dubbed", False)
        if pref_lang != "all":
            if pref_lang == "orig" and not is_dubbed:
                lang_match = 1
    ...
    for stream_data in stream_results:
        ...
        if pref_lang != "all":
            if pref_lang == "orig" and is_dubbed:
                # User wants original audio — skip dubbed streams
                continue
            elif pref_lang != "orig":
                # Specific language (hi/en/etc.) — match by audio_lang
